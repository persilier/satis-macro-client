...
const StaffForm = (props) => {
  ...
  if (id) {
    if (
      !(
        verifyPermission(props.userPermissions, "update-staff-from-any-unit") ||
        verifyPermission(props.userPermissions, "update-staff-from-my-unit") ||
        verifyPermission(
    ...
    if (
      !(
        verifyPermission(props.userPermissions, "store-staff-from-any-unit") ||
        verifyPermission(props.userPermissions, "store-staff-from-my-unit") ||
        verifyPermission(
          props.userPermissions,
      ...
      if (id) {
        axios
          ...
          .then((response) => {
            ...
            if (
              ...
              setInstitution({
                value: response.data.staff.institution.id,
                label: response.data.staff.institution.name,
              });
            }
            setPosition({
              value: response.data.staff.position.id,
              label: response.data.staff.position.name["fr"],
            });
            ...
            setUnit({
              value: response.data.staff.unit.id,
              label: response.data.staff.unit.name["fr"],
              lead: response.data.staff.unit.lead,
            });
            setData(newData);
          })
          .catch((error) => {
        ...
        axios
          ...
          .then((response) => {
            ...
            setPositions(response.data.positions);
            if (response.data.units) setUnits(formatUnits(response.data.units));
          })
          .catch((error) => {
            //console.log("something is wrong");
          });
  ...
  const onChangeUnit = (selected) => {
    ...
    if (selected === null) newData.is_lead = 0;
    setUnit(selected);
    setData(newData);
  };
    ...
    if (selected) {
      if (verifyTokenExpire()) {
        if (
          ...
          axios
            .get(`${appConfig.apiDomaine}/institutions/${selected.value}/units`)
            .then((response) => {
              newData.unit_id = "";
              setUnit(null);
  ...
  const confirmIsLead = (config, value) => {
    ConfirmLead.fire(config).then((result) => {
      if (result.value) {
        changeOption(value);
      }
    });
  ...
  const handleOptionChange = (e) => {
    if (unit) {
      ...
      ToastBottomEnd.fire(
        toastErrorMessageWithParameterConfig(t("Veuillez choisir une unité"))
      );
    }
  };
        ...
        axios
          ...
          .catch((errorRequest) => {
            ...
            if (errorRequest.response.data.error.staff) {
              ...
              ToastBottomEnd.fire(
                toastErrorMessageWithParameterConfig(
                  errorRequest.response.data.error.staff.identite.lastname +
                    " " +
                    errorRequest.response.data.error.staff.identite.firstname +
                    ": " +
                    errorRequest.response.data.error.message
                )
              );
                    ...
                    <div className="kt-section">
                      <div className="kt-section__body">
                        ...
                        >
                          <div className="col">
                            <label htmlFor="position">
                              {t("Fonction")} <InputRequire />
                            </label>
                            <Select
                              isClearable
                              value={position}
                              placeholder={t(
                                "Veuillez sélectionner la fonction"
                              )}
                        ...
                        <div className="form-group row">
                          ...
                          >
                            <label
                              className="col-xl-6 col-lg-6 col-form-label mt-4"
                              htmlFor="name"
                            >
                              {t("Responsable de l'unité")} <InputRequire />
                            </label>
                            <div className="col-lg-6 col-xl-6 kt-radio-inline">
                              <label className="kt-radio mt-4">
                                <input
                                  ...
                                  checked={optionOne === data.is_lead}
                                />{" "}
                                {t("OUI")}
                                <span />
                              </label>
                              <label className="kt-radio">
                                <input
                                  ...
                                  value={optionTwo}
                                  onChange={handleOptionChange}
                                  checked={optionTwo === data.is_lead}
                                />{" "}
                                {t("NON")}
                                <span />
                              </label>
