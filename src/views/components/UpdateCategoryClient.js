...
const UpdateCategoryClient = () => {
    ...
    const {categoryelemtid} = useParams();
    ...
    useEffect(() => {
        ...
        axios.get(`http://127.0.0.1:8000/category-clients/${categoryelemtid}`)
            .then(response => {
    ...
    const updateCatClient = () => {
        setLoadRequest(true);
        axios({
            method: 'put',
            url: `http://127.0.0.1:8000/category-clients/${categoryelemtid}`,
            data: {
                name:name,
