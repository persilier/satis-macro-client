...
const DashboardStatClaim = (props) => {
  ...
  useEffect(() => {
    ...
    newSatisfaction.options.xaxis.categories = Object.values(
      newData.map((label) => label.month)
    );
    for (let i = 0; i <= satisfiedData.length - 1; i++) {
      newSatisfaction.series[i].data = Object.values(newData).map(
        (serie) => serie["data" + i]
      );
    }
