      ...
      {treatments?.map?.((item, index) => (
        <div className="kt-wizard-v2__review-item mb-3" key={index}>
          {!item.invalidated_reason ? (
            <div className="kt-wizard-v2__review-title">
              <h5>
                <strong>{t("Traitement validé")}</strong>
              </h5>
            </div>
          ) : null}
          ...
          <div className="kt-wizard-v2__review-content jumbotron px-2 py-2 mb-0">
            {item.unfounded_reason ? (
              <>
                ...
                <span className="mx-2">{item.unfounded_reason}</span>
                <br />
              </>
            ) : null}
            ...
            item.comments ? (
              <>
                <strong>{t("Montant")}</strong>:{" "}
                <span
                  className={`mx-2 ${
                    !item.invalidated_reason ? "text-success" : ""
                  }`}
                >
                  {item.amount_returned ? item.amount_returned : "-"}
                </span>
                <br />
                ...
                <br />
                <strong>{t("Mesure preventive")}</strong>:{" "}
                <span
                  className={`mx-2 ${
                    !item.invalidated_reason ? "text-success" : ""
                  }`}
                >
                  {item.preventive_measures ? item.preventive_measures : "-"}
                </span>
                <br />
                <strong>{t("Commentaire")}</strong>:{" "}
                <span
                  className={`mx-2 ${
                    !item.invalidated_reason ? "text-success" : ""
                  }`}
            ...
            {item.solved_at ? (
              <>
                ...
                >
                  {formatDateToTimeStampte(item.solved_at)}
                </span>
                <br />
              </>
            ) : null}
            ...
            {item.validated_at ? (
              <>
                <strong>{t("Date de validation du traitement")}</strong>:{" "}
                <span
                  className={`mx-2 ${
                    !item.invalidated_reason ? "text-success" : ""
                  }`}
                >
                  {formatDateToTimeStampte(item.validated_at)}
                </span>
                <br />
              </>
            ) : null}
