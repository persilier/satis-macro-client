...
const ParametersComponentEdit = (props) => {
  ...
  const formatState = (params, paramData = null) => {
    ...
    const newError = {};
    params.map((param) => {
      newState[`params_${param}`] = paramData
        ? paramData[param].type === "image"
          ? paramData[param].value !== null
            ? paramData[param].value?.url
            : null
      ...
      if (paramData) {
        paramData[param].type === "image"
          ? (newDatalogo[`params_${param}`] = false)
          : delete newDatalogo[`params_${param}`];
      }
    });
    setData(newState);
  ...
  useEffect(() => {
    if (verifyTokenExpire()) {
      axios.get(appConfig.apiDomaine + `/components/${id}`).then((response) => {
  ...
  const saveData = (e) => {
    ...
    for (let i = 0; i < cle.length; i++) {
      ...
      values.map((param) =>
        param[cle[i]] === false ? delete newData[cle[i]] : ""
      );
    }
                    ...
                    <div className="kt-portlet__body">
                      ...
                      {Object.keys(error).length
                        ? Object.values(data).map((param, index) =>
                            dataType[Object.keys(dataType)[index]] ===
                            "image" ? (
                              <div
                                key={index}
                                className={
                              ...
                              >
                                <label
                                  ...
                                  htmlFor={Object.keys(data)[index]}
                                >
                                  {Object.keys(data)[index].slice(7)}
                                  <InputRequire />
                                </label>
                                ...
                                <div className="col-lg-9 col-xl-6">
                                  ...
                                  >
                                    ...
                                    {error[Object.keys(error)[index]].length
                                      ? error[Object.keys(error)[index]].map(
                                          (error, index) => (
                                            <div
                                              key={index}
                              ...
                              >
                                ...
                                <div className="col-lg-9 col-xl-6">
                                  <input
                                    ...
                                    onChange={(e) =>
                                      handleChange(e, Object.keys(data)[index])
                                    }
                                  />
                                  {error[Object.keys(error)[index]].length
                                    ? error[Object.keys(error)[index]].map(
