...
const ReasonSatisfaction = (props) => {
  ...
  const onClick = (e) => {
    ...
    axios
      .put(props.getEndPoint + `/${props.getId}`, data)
      .then((response) => {
  ...
  return ready ? (
    <div>
      ...
      >
        <label>
          {t("Client Satisfait?")} <InputRequire />
        </label>
      ...
      <div
        className={
          error.note.length ? "form-group row validated" : "form-group row"
        }
      >
        <label className="col-xl-3 col-lg-3 col-form-label" htmlFor="raison">
          {t("Note")}
        </label>
        <div className="col-lg-9 col-xl-6">
          <input
            ...
            max={5}
            className={
              error.note.length ? "form-control is-invalid" : "form-control"
            }
