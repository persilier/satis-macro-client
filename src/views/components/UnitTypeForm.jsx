...
const UnitTypeForm = (props) => {
  ...
  useEffect(() => {
    async function fetchData() {
      if (id) {
        await axios
          .get(`${appConfig.apiDomaine}/unit-types/${id}/edit`)
          .then(({ data }) => {
            const newData = {
              ...
              description: data.unitType.description
                ? data.unitType.description.fr
                : "",
            };
            setData(newData);
          })
  ...
  const onSubmit = (e) => {
    ...
    if (verifyTokenExpire()) {
      if (id) {
        axios
          .put(`${appConfig.apiDomaine}/unit-types/${id}`, defaultDataPost)
          .then((response) => {
          ...
          .catch((errorRequest) => {
            ...
            if (errorRequest.response.data.error.is_editable)
              ToastBottomEnd.fire(
                toastErrorMessageWithParameterConfig(
                  errorRequest.response.data.error.is_editable[0]
                )
              );
            else {
              setError({
        ...
        axios
          .post(`${appConfig.apiDomaine}/unit-types`, defaultDataPost)
          .then((response) => {
            setStartRequest(false);
            setError(defaultError);
            setData(defaultData);
                ...
                <div className="kt-portlet__head">
                  <div className="kt-portlet__head-label">
                    <h3 className="kt-portlet__head-title">
                      {id
                        ? t("Modification de type d'unité")
                        : t("Ajout d'un type d'unité")}
                    </h3>
                  </div>
                </div>
                ...
                <form method="POST" className="kt-form">
                  <div className="kt-form kt-form--label-right">
                    <div className="kt-portlet__body">
                      <div
                        className={
                          error.can_be_target.length || error.can_treat.length
                        ...
                        <div className="col-lg-9 col-xl-6">
                          <div className="kt-checkbox-inline">
                            ...
                            <label className="kt-checkbox">
                              <input
                                type="checkbox"
                                checked={data.can_treat}
                                onChange={handleCanTreatChange}
                              />{" "}
                              {t("Peut résoudre une réclamation ?")}
                              <span />
                      ...
                      >
                        ...
                        <div className="col-lg-9 col-xl-6">
                          <input
                            ...
                            placeholder={t(
                              "Veuillez entrer le nom du type d'unité"
                            )}
                            value={data.name}
