...
const ColToValidate = (props) => {
  ...
  const filterByUnit = () => {
    currentFilterData = currentFilterData.filter(
      (claim) =>
        claim.active_treatment.responsible_unit_id.indexOf(
          props.filterUnit.value
        ) >= 0
    );
  };
  ...
  const filterByStaff = () => {
    currentFilterData = currentFilterData.filter((claim) => {
      return (
        claim.active_treatment.responsible_staff_id.indexOf(
          props.filterStaff.value
        ) >= 0
      );
    });
  };
  ...
  if (props.filterStaff) filterByStaff();
