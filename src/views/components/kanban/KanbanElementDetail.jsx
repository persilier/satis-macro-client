        ...
        if (verifyTokenExpire()) {
          axios
            ...
            .then((response) => {
              ...
              ToastBottomEnd.fire(
                toastSuccessMessageWithParameterConfig(
                  "Réclamation revoquer avec succès"
                )
              );
              setTimeout(() => {
            ...
            .catch((error) => {
              ...
              ToastBottomEnd.fire(
                toastErrorMessageWithParameterConfig(
                  "Echec de revocation de la réclamation"
                )
              );
    ...
    >
      ...
      <div className="kt-portlet__body kt-portlet__body--fit-top">
        <p style={{ textAlign: "left" }}>
          {t("La réclamation dont l'objet est")}{" "}
          <strong>
            {claim?.claim_object
              ? claim?.claim_object
          ...
          {t("est")} <br /> {t("reçu le")}{" "}
          <strong>
            {moment(new Date(claim?.created_at))?.format("DD/MM/YYYY")}
          </strong>{" "}
          <br />
        ...
        {verifyPermission(userPermissions, "revoke-claim") &&
          ["incomplete", "full"].includes(claim.status) && (
            <>
              {revokeLoad ? (
                <button
                  className="btn btn-outline-danger btn-sm kt-spinner kt-spinner--v2 kt-spinner--sm kt-spinner--primary mb-2 text-uppercase"
                  disabled={true}
                >
                  {t("Chargement")}
                </button>
                ...
                <button
                  onClick={revoke}
                  className="btn btn-outline-danger btn-sm text-uppercase mb-2"
                >
                  {t("Revoquer")}
                </button>
              )}
        ...
        {verifyPermission(userPermissions, "revive-staff") && (
          <button
            ...
            type="button"
            className="btn btn-outline-warning btn-sm text-uppercase"
          >
            {t("Relancer")}
          </button>
        )}
