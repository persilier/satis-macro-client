...
const endPointConfig = {
  ...
  HUB: {
    plan: "HUB",
    edit: (id) =>
      `${appConfig.apiDomaine}/without-client/claims-incompletes/${id}/edit`,
    update: (id) =>
      `${appConfig.apiDomaine}/without-client/claims-incompletes/${id}`,
  },
};
...
const IncompleteClaimsEdit = (props) => {
  ...
  document.title = ready
    ? t("Satis client - Complétion plaintes incomplètes")
    : "";
  const { id } = useParams();
  if (
    !(
  ...
  useEffect(() => {
    async function fetchData() {
      await axios.get(endPoint.edit(`${id}`)).then((response) => {
        // console.log(response.data, "DATA")
        const newIncompleteClaim = {
          claimer_id: response.data.claim.claimer_id,
          firstname:
            response.data.claim.claimer !== null &&
            response.data.claim.claimer.firstname !== null
              ? response.data.claim.claimer.firstname
              : "",
          lastname:
            response.data.claim.claimer !== null &&
            response.data.claim.claimer.lastname !== null
              ? response.data.claim.claimer.lastname
              : "",
          sexe:
            response.data.claim.claimer !== null &&
            response.data.claim.claimer.sexe !== null
              ? response.data.claim.claimer.sexe
              : "",
          telephone:
            response.data.claim.claimer !== null &&
            response.data.claim.claimer.telephone !== null
              ? response.data.claim.claimer.telephone
              : [],
          email:
            response.data.claim.claimer === null ||
            response.data.claim.claimer.email === null
              ? []
              : response.data.claim.claimer.email,
          ville:
            response.data.claim.claimer === null ||
          ...
          raison_sociale:
            ...
            response.data.claim.claimer.raison_sociale === null
              ? ""
              : response.data.claim.claimer.raison_sociale,
          ...
          claimer_expectation:
            response.data.claim.claimer_expectation === null
              ? ""
              : response.data.claim.claimer_expectation,
          description: response.data.claim.description,
          lieu:
            response.data.claim.lieu !== null ? response.data.claim.lieu : null,
          amount_currency_slug: response.data.claim.amount_currency_slug
            ? response.data.claim.amount_currency_slug
            : "",
        ...
        if (
          ...
          setRelationships(
            formatSelectOption(response.data.relationships, "name", "fr")
          );
        setAccounts(
        ...
        setCurrencies(
          formatSelectOption(response.data.currencies, "name", "fr", "slug")
        );
        setChannels(
          formatSelectOption(response.data.channels, "name", "fr", "slug")
        );
        setResponseChannels(
          formatSelectOption(
            filterChannel(response.data.channels, RESPONSE_CHANNEL),
            "name",
            "fr",
            "slug"
          )
        ...
        if (
          response.data.claim.claim_object &&
          response.data.claim.claim_object.claim_category !== null
        ) {
          setClaimCategory({
      ...
      await axios
        .get(
          appConfig.apiDomaine + "/components/retrieve-by-name/register_claim"
        )
        .then((response) => {
  ...
  const onChangeUnit = (selected) => {
    setUnit(selected);
    const newData = { ...data };
    if (selected.value !== "other") newData.unit_targeted_id = selected.value;
    else newData.unit_targeted_id = "";
    setData(newData);
  };
  ...
  const onChangeEventOccuredAt = (e) => {
    ...
    if (new Date(e.target.value) >= new Date()) {
      ToastBottomEnd.fire(
        toastErrorMessageWithParameterConfig(t("Date invalide"))
      );
      newData.event_occured_at = "";
    } else newData.event_occured_at = e.target.value;
    setData(newData);
  };
  ...
  const formatFormData = (newData) => {
    ...
    for (const key in newData) {
      ...
      } else formData.set(key, newData[key]);
    }
    return formData;
  };
  ...
  const onSubmit = (e) => {
    ...
    if (!newData.account_targeted_id) delete newData.account_targeted_id;
    if (!newData.account_number) delete newData.account_number;

    if (!newData.amount_disputed) delete newData.amount_disputed;
    if (!newData.amount_currency_slug) delete newData.amount_currency_slug;
    if (
      !verifyPermission(
        props.userPermissions,
        "update-claim-incomplete-without-client"
    ...
    if (verifyTokenExpire()) {
      axios
        .post(endPoint.update(`${id}`), formatFormData(newData))
        .then((response) => {
          setStartRequest(false);
          ToastBottomEnd.fire(
            toastSuccessMessageWithParameterConfig(t("Succès de la complétion"))
          );
        ...
        .catch((error) => {
          ...
          if (error.response.data.code === 422) {
            ...
            ToastBottomEnd.fire(
              toastAddErrorMessageConfig(error.response.data.error)
            );
          } else {
            setStartRequest(false);
    ...
    load ? (
      ...
      >
        ...
        <div className="kt-container  kt-container--fluid  kt-grid__item kt-grid__item--fluid">
          <InfirmationTable
            information={
              "Formulaire d'enregistrement d'une réclamation. " +
              t(
                "Utilisez ce formulaire pour completer les réclamations imcomplètes de vos clients"
              )
            }
          />
          <div className="row">
                  ...
                  <div className="kt-portlet__body">
                    ...
                    verifyPermission(
                      ...
                      >
                        ...
                        <div className="col-lg-9 col-xl-6">
                          ...
                          />
                          {error.institution_targeted_id.length
                            ? error.institution_targeted_id.map(
                                (error, index) => (
                      ...
                      <div className="kt-section kt-section--first">
                        <div className="kt-section__body">
                          <h3 className="kt-section__title kt-section__title-lg">
                            {componentData
                              ? componentData.params.fr.info_cible.value + ":"
                              : ""}
                          </h3>
                          ...
                          {(typeof data.type_client == "undefined" ||
                    ...
                    <div className="kt-section">
                      <div className="kt-section__body">
                        <h3 className="kt-section__title kt-section__title-lg">
                          {componentData
                            ? componentData.params.fr.info_reclamation.value
                            : ""}
                        </h3>
                        {!verifyPermission(
                          props.userPermissions,
                          ...
                          <div className="form-group row">
                            ...
                            >
                              ...
                              <Select
                                ...
                                placeholder={
                                  componentData
                                    ? componentData.params.fr.unite_placeholder
                                        .value
                                    : ""
                                }
                                value={unit}
                            ...
                            >
                              <label htmlFor="account">
                                {componentData
                                  ? componentData.params.fr.compte.value
                                  : ""}{" "}
                                {isRequire.account_targeted_id ? (
                                  <InputRequire />
                                ) : (
                                  ""
                                )}
                        ...
                        <div className="form-group row">
                          ...
                          >
                            <label htmlFor="receptionChannel">
                              {componentData
                            ...
                            <Select
                              ...
                              placeholder={
                                componentData
                                  ? componentData.params.fr
                                      .canal_reception_placeholder.value
                                  : ""
                              }
                            ...
                            />
                            {error.request_channel_slug.length
                              ? error.request_channel_slug.map(
                                  (error, index) => (
                                    <div
                                      key={index}
                                      className="invalid-feedback"
                          ...
                          >
                            ...
                            <Select
                              ...
                              placeholder={
                                componentData
                                  ? componentData.params.fr
                                      .canal_reponse_placeholder.value
                                  : ""
                              }
                            ...
                            />
                            {error.response_channel_slug.length
                              ? error.response_channel_slug.map(
                                  (error, index) => (
                                    <div
                                      key={index}
                                      className="invalid-feedback"
                        ...
                        <div className="form-group row">
                          ...
                          >
                            <label htmlFor="claimObject">
                              {componentData
                                ? componentData.params.fr.object.value
                                : ""}
                              <InputRequire />
                            </label>
                            <Select
                              ...
                              placeholder={
                                componentData
                                  ? componentData.params.fr.object_placeholder
                                      .value
                                  : ""
                              }
                              value={claimObject}
                          ...
                          <div
                            className={
                              error.lieu.length ? "col validated" : "col"
                            }
                          >
                            <label htmlFor="lieu">
                              {componentData
                                ? componentData.params.fr.lieu.value
                                : ""}{" "}
                            </label>
                            <input
                              // disabled={!disabledInput}
                              ...
                              placeholder={
                                componentData
                                  ? componentData.params.fr.lieu_placeholder
                                      .value
                                  : ""
                              }
                              value={data.lieu}
                        ...
                        <div className="form-group row">
                          ...
                          >
                            <label htmlFor="amount_claim">
                              {componentData
                                ? componentData.params.fr.montant.value
                                : ""}{" "}
                              (
                              <strong className="text-danger">
                                {t("Laisser vide si pas de montant")}
                              </strong>
                              ){" "}
                              {isRequire.amount_disputed ? (
                                <InputRequire />
                              ) : (
                                ""
                              )}
                            ...
                            <input
                              ...
                              placeholder={
                                componentData
                                  ? componentData.params.fr.montant_placeholder
                                      .value
                                  : ""
                              }
                          ...
                          >
                            ...
                            <Select
                              ...
                              placeholder={
                                componentData
                                  ? componentData.params.fr.devise_placeholder
                                      .value
                                  : ""
                              }
                              value={currency}
                            ...
                            />
                            {error.amount_currency_slug.length
                              ? error.amount_currency_slug.map(
                                  (error, index) => (
                                    <div
                                      key={index}
                                      className="invalid-feedback"
                        ...
                        <div className="form-group row">
                          ...
                          >
                            <label htmlFor="date">
                              {componentData
                                ? componentData.params.fr.date.value
                                : ""}
                              <InputRequire />
                            </label>
                            <input
                              ...
                              placeholder={
                                componentData
                                  ? componentData.params.fr.date_placeholder
                                      .value
                                  : ""
                              }
                          ...
                          {verifyPermission(
                            ...
                            >
                              <label htmlFor="relationship">
                                {componentData
                                  ? componentData.params.fr.relation.value
                                  : ""}{" "}
                                {isRequire.relationship_id ? (
                                  <InputRequire />
                                ) : (
                                  ""
                                )}
                          ...
                          <div className="col">
                            <label htmlFor="file">
                              {componentData
                                ? componentData.params.fr.piece.value
                                : ""}{" "}
                              {isRequire.file ? <InputRequire /> : ""}
                            </label>
                            <input
                              ...
                              placeholder={
                                componentData
                                  ? componentData.params.fr.piece_placeholder
                                      .value
                                  : ""
                              }
                              id="customFile"
                        ...
                        <div className="form-group row">
                          <div
                            className={
                              error.description.length ? "col validated" : "col"
                            }
                          >
                            <label htmlFor="description">
                              {componentData
                                ? componentData.params.fr.description.value
                                : ""}
                              {isRequire.description ? <InputRequire /> : ""}
                            </label>
                          ...
                          >
                            <label htmlFor="claimer_expectation">
                              {componentData
                            ...
                            <textarea
                              ...
                              placeholder={
                                componentData
                                  ? componentData.params.fr.attente_placeholder
                                      .value
                                  : ""
                              }
                    ...
                    <div className="kt-section">
                      <div className="kt-section__body">
                        <h3 className="kt-section__title kt-section__title-lg">
                          {componentData
                            ? componentData.params.fr.last_titre.value
                            : ""}
                          <InputRequire />
                        </h3>
                        ...
                        <div className="form-group row">
                          <label className="col-3 col-form-label">
                            {componentData
                              ? componentData.params.fr.question.value
                              : ""}
                          </label>
                          <div className="col-9">
                            <div className="kt-radio-inline">
                              <label className="kt-radio">
                                <input
                                  ...
                                  checked={option1 == data.is_revival}
                                />{" "}
                                {componentData
                                  ? componentData.params.fr.reponse_oui.value
                                  : ""}
                                <span />
                              </label>
                              <label className="kt-radio">
                                <input
                                  ...
                                  value={option2}
                                  onChange={handleOptionChange}
                                  checked={option2 == data.is_revival}
                                />{" "}
                                {componentData
                                  ? componentData.params.fr.reponse_non.value
                                  : ""}
                                <span />
                              </label>
                            </div>
