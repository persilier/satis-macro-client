...
const InstitutionForm = (props) => {
  ...
  useEffect(() => {
    if (verifyTokenExpire()) {
      if (verifyPermission(props.userPermissions, "store-any-institution")) {
        axios
          .get(appConfig.apiDomaine + "/any/institutions/create")
          .then((response) => {
            setDefaultCurrencyData(
              response.data.currencies.length
      ...
      if (id) {
        axios
          .get(appConfig.apiDomaine + `/any/institutions/${id}`)
          .then((response) => {
            const newInstitution = {
              default_currency_slug:
                response.data.default_currency_slug !== null
                  ? response.data.default_currency_slug
                  : "",
  ...
  const onSubmit = (e) => {
    ...
    if (verifyTokenExpire()) {
      if (id) {
        ...
        axios
          .post(appConfig.apiDomaine + `/any/institutions/${id}`, formData)
          .then((response) => {
        ...
        axios
          .post(appConfig.apiDomaine + `/any/institutions`, formData)
          .then((response) => {
            setError(defaultError);
            setStartRequest(false);
            setData(defaultData);
                                  ...
                                  >
                                    ...
                                    <div className="col-lg-9 col-xl-6">
                                      <Select
                                        ...
                                        options={
                                          defaultCurrencyData.length
                                            ? defaultCurrencyData.map(
                                                (name) => name
                                              )
                                            : ""
                                        }
                                      />
                                      {error.default_currency_slug.length
                                        ? error.default_currency_slug.map(
                                            (error, index) => (
                                              <div
                                                key={index}
