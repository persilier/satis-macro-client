...
const Logs = (props) => {
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      setLoad(true);
      axios
        .get(`${appConfig.apiDomaine}/activity-log/create`)
        .then((response) => {
          const logData = response.data.logs;
          setNextUrl(logData.next_page_url);
          setLogs(logData.data);
          setActors(
            response.data.filters.causers.map((item) => {
              return {
                value: item.id,
          ...
          setActions(
            Object.keys(logActions).map((item) => {
              return {
                value: item,
  ...
  const showMore = () => {
    if (
      endIndex / onePageNumber <
      forceRound(logs.length / onePageNumber) - 1
    ) {
        ...
        if (nextUrl !== null) {
          ...
          axios
            ...
            .then((response) => {
              ...
              setLogs((l) => {
                return [...l, ...response.data.logs.data];
              });
              setEndIndex((i) => i + onePageNumber);
            })
            .catch((error) => {
  ...
  const filterLogs = (e) => {
    ...
    if (verifyTokenExpire()) {
      ...
      if (!sendData.causer_id) delete sendData.causer_id;
      if (!sendData.log_action) delete sendData.log_action;
      axios
        .get(`${appConfig.apiDomaine}/activity-log`, { params: sendData })
        .then((response) => {
          ...
          ToastBottomEnd.fire(
            toastSuccessMessageWithParameterConfig(t("Succès du filtrage"))
          );
          setLoadFilter(false);
        ...
        .catch((error) => {
          ToastBottomEnd.fire(
            toastErrorMessageWithParameterConfig(t("Echec du filtrage"))
          );
    ...
    ? verifyPermission(props.userPermissions, "activity-log") && (
        ...
        >
          ...
          <div className="kt-container  kt-container--fluid  kt-grid__item kt-grid__item--fluid">
            <InfirmationTable
              information={t("Historique des activitées du système")}
            />

            <div className="kt-portlet">
              <HeaderTablePage title={t("Journal des activités")} />
              ...
              <div className="kt-portlet__body">
                ...
                >
                  <form
                    className="kt-form kt-form--fit kt-margin-b-20"
                    onSubmit={filterLogs}
                  >
                        ...
                        <div className="form-group row">
                          <div className="col">
                            ...
                            <input
                              ...
                              className={`form-control ${
                                error.date_start.length ? "is-invalid" : ""
                              }`}
                            />
                            {error.date_start.length !== 0 && (
                              <>
                                {error.date_start.map((item, index) => (
                                  <p key={index} className="invalid-feedback">
                                    {item}
                                  </p>
                                ))}
                              </>
                          ...
                          <div className="col">
                            ...
                            <input
                              ...
                              type="date"
                              className={`form-control ${
                                error.date_end.length ? "is-invalid" : ""
                              }`}
                            />
                          ...
                          <div key={index}>
                            <div className="tab-content">
                              <div className="d-flex justify-content-between">
                                <h5>
                                  {item.causer
                                    ? item.causer.identite
                                      ? item.causer.identite.lastname +
                                        " " +
                                        item.causer.identite.firstname
                                      : ""
                                    : ""}
                                </h5>
                                <span>
                                  {formatDateToTimeStampte(item.created_at)}
                                </span>
                              </div>
                              <div style={{ marginTop: "1%" }}>
                                <strong>{t("Action")}: </strong>
                                <span className="mx-2">
                                  {showAction(item.log_action)}
                                </span>
                              </div>
