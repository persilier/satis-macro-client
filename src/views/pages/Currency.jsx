...
const Currency = (props) => {
  ...
  useEffect(() => {
    async function fetchData() {
      await axios
        .get(`${appConfig.apiDomaine}/currencies`)
        .then((response) => {
          setNumberPage(
  ...
  const filterShowListBySearchValue = (value) => {
    ...
    newCurrencies = newCurrencies.filter(
      (el) =>
  ...
  const deleteCurrency = (currencyId, index) => {
    DeleteConfirmation.fire(confirmDeleteConfig()).then((result) => {
      if (verifyTokenExpire()) {
        if (result.value) {
          axios
            .delete(`${appConfig.apiDomaine}/currencies/${currencyId}`)
            .then((response) => {
                  ...
                  <div className="row">
                    <div className="col-sm-12 col-md-5">
                      ...
                      >
                        ...
                        {t("sur")} {currencies.length} {t("données")}
                      </div>
                    </div>
                    {showList.length ? (
