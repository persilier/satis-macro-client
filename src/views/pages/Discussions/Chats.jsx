...
const Chats = (props) => {
  ...
  useEffect(() => {
    if (verifyTokenExpire()) {
      axios
        .get(appConfig.apiDomaine + "/discussions")
        .then((response) => {
  ...
  useEffect(() => {
    if (userDataJson.staff.identite_id && idChat) {
      window.Echo.private(
        `Satis2020.ServicePackage.Models.Identite.${userDataJson.staff.identite_id}`
  ...
  const onChangeDiscussion = (id, status) => {
    setChatStatus(status);
    async function fetchData() {
      await axios
        .get(appConfig.apiDomaine + `/discussions/${id}/staff`)
        .then((response) => {
          setActiveChat(true);
  ...
  const getListMessage = (id) => {
    async function fetchData() {
      await axios
        .get(appConfig.apiDomaine + `/discussions/${id}/messages`)
        .then((response) => {
  ...
  const deleteContributor = (chatsId, index) => {
    DeleteConfirmation.fire(confirmDeleteConfig()).then((result) => {
      if (result.value) {
        if (verifyTokenExpire()) {
          axios
            .delete(appConfig.apiDomaine + `/discussions/${chatsId}`)
            .then((response) => {
  ...
  const addItem = (e) => {
    e.preventDefault();
    let newData = { ...data };
    if (!newData.files.length) delete newData.files;
    if (newData.parent_id === "") delete newData.parent_id;
    ...
    if ((data.text !== "" && idChat) || (data.files !== [] && idChat)) {
      ...
      if (verifyTokenExpire()) {
        axios
          .post(
            appConfig.apiDomaine + `/discussions/${idChat}/messages`,
    ...
    DeleteConfirmation.fire(confirmDeleteConfig()).then((result) => {
      if (result.value) {
        if (verifyTokenExpire()) {
          axios
            .delete(
              appConfig.apiDomaine + `/discussions/${idChat}/messages/${key}`
            )
            .then((response) => {
              getListMessage(idChat);
              const filteredItems = listChatMessages.filter(
                (item) => item.key !== key
              );
                                ...
                                <div className="kt-widget__item">
                                  ...
                                  <div className="kt-widget__action">
                                    <span className="kt-widget__date">
                                      {moment(chat.created_at).format("ll")}
                                    </span>
                                    {idChat === chat.id}
                                    {/*<span*/}
                                        ...
                                        <ul className="kt-nav">
                                          <li className="kt-nav__item">
                                            <Link
                                              to={
                                                chat.id
                                                  ? `/treatment/chat/contributor/${chat.id}`
                                                  : ""
                                              }
                                          ...
                                          chat.created_by ? (
                                            <li className="kt-nav__item">
                                              <a
                                                ...
                                                className="kt-nav__link"
                                                onClick={(e) =>
                                                  deleteContributor(chat.id, i)
                                                }
                                              >
                                                <i className="kt-nav__link-icon flaticon-delete"></i>
                                                <span className="kt-nav__link-text">
                                                  {t("Supprimer un Tchat")}
                                                </span>
                                              </a>
                                            </li>
                                          ) : null}
          ...
          >
            <div className="kt-chat" id="kt-chat">
              <div className="kt-portlet kt-portlet--head-lg- ">
                ...
                <div className="kt-portlet__body">
                  <div
                    className="kt-scroll kt-scroll--pull ps ps--active-y overflow-auto"
                    id="kt-scroll"
                    ...
                    <div className="kt-chat__editor">
                      {data.parent_id ? (
                        <div className="message_target">
                          <i
                            className="d-flex justify-content-end flaticon-close"
                      ...
                      {data.files
                        ? data.files.map((file, i) => (
                            <div className="message_target" key={i}>
                              ...
                              ></i>
                              <img
                                src="/assets/media/users/file-icon.png"
                                alt=""
                                style={{
                                  maxWidth: "55px",
                                  maxHeight: "55px",
