    ...
    DeleteConfirmation.fire(confirmDeleteConfig()).then((result) => {
      if (result.value) {
        if (verifyTokenExpire()) {
          axios
            .delete(
              appConfig.apiDomaine + `/discussions/${id}/staff/${contributorId}`
            )
            .then((response) => {
      ...
      <tr key={index} role="row" className="odd">
        ...
        <td>
          {user.identite.email
            ? user.identite.email.map((mail, index) =>
                index === user.identite.email.length - 1
                  ? mail
                  : mail + " " + <br /> + " "
              )
            : null}
        ...
        {userDataJson.staff.id === responseData.created_by.id ? (
          <td style={{ textAlign: "center" }}>
            {user.id === responseData.created_by.id ? null : (
              <button
    ...
    verifyPermission(props.userPermissions, "list-discussion-contributors") ? (
      ...
      >
        ...
        <div className="kt-container  kt-container--fluid  kt-grid__item kt-grid__item--fluid">
          <InfirmationTable information={t("Liste des participants")} />
