...
const CommitteeAdhoc = (props) => {
  ...
  if (
    !(
      verifyPermission(props.userPermissions, "list-treatment-board") &&
      props.activePilot
    )
  )
  ...
  useEffect(() => {
    async function fetchData() {
      axios
        .get(`${appConfig.apiDomaine}/treatments-boards`)
        .then((response) => {
  ...
  const printBodyTable = (committee, index, id) => {
    return (
      <tr key={index} role="row" className="odd">
        ...
        <td>
          {" "}
          <span style={{ marginRight: "5px" }}>{committee.members.length}</span>
          <HtmlDescriptionCommittee
            onClick={() =>
              showMemberModal(committee.members ? committee.members : "-")
            }
          />
        </td>
        <td className={"d-contents"} style={{ display: "reverts" }}>
          {committee.claim && committee.claim.reference ? (
            committee.claim.reference
          ) : (
            <td style={{ color: "red", borderTop: "1px solid transparent" }}>
              <i className="kt-menu__link-icon flaticon-questions-circular-button" />
            </td>
          )}
        </td>
        <td>
          {committee.claim ? (
            committee.claim.claim_object.name["fr"]
          ) : (
            <td style={{ color: "red", borderTop: "1px solid transparent" }}>
              <i className="kt-menu__link-icon flaticon-questions-circular-button" />
            </td>
        ...
        <td style={{ textAlign: "center" }}>
          <HtmlDescription
            onClick={() =>
              showModal(
                committee.claim && committee.claim.description
                  ? committee.claim.description
                  : "-"
              )
            }
          />
        ...
        <td className={"text-center"}>
          {verifyPermission(props.userPermissions, "update-treatment-board") ? (
            <Link
              to={`/settings/committee/${committee.id}/edit`}
      ...
      >
        ...
        <div className="kt-container  kt-container--fluid  kt-grid__item kt-grid__item--fluid">
          ...
          <div className="kt-portlet">
            <HeaderTablePage title={t("Liste des comités Ad'hoc")} />
                ...
                >
                  ...
                  <div className="row">
                    <div className="col-sm-12">
                      ...
                      />
                      <MemberDescriptionModal
                        title={t("Liste des membres du comité")}
                        message={memberList}
                      />
                    </div>
                  </div>
