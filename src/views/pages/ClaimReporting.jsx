...
const ClaimReporting = (props) => {
  ...
  const [pdfTitle, setPdfTitle] = useState(
    `reporting_${new Date().getFullYear()}`
  );
  ...
  const calculateTotal = () => {
    fetchData.statistiqueObject.map((row) => {
      row.claim_objects.map((elRow) => {
        ...
        totalAwaitingTreatment =
          totalAwaitingTreatment + elRow.awaitingTreatment;
        totalToValidate = totalToValidate + elRow.toValidate;
        totalToMeasureSatisfaction =
          totalToMeasureSatisfaction + elRow.toMeasureSatisfaction;
        totalPercentage = totalPercentage + elRow.percentage;
      });
    });
    totalPercentage = parseFloat(
      (isNaN(totalPercentage / fetchData.statistiqueObject.length)
        ? 0
        : totalPercentage / fetchData.statistiqueObject.length
      ).toFixed(2)
    );
  };

  if (fetchData) calculateTotal();

  const applyPossibleFilter = (data) => {
    const newPossibleFilter = { ...possibleFilter };

    newPossibleFilter.days =
      Object.keys(data.days.claims_received).length <= 17;
    newPossibleFilter.weeks =
      Object.keys(data.weeks.claims_received).length <= 5;
  ...
  const getGraphTwoOptions = (dataGraphTwo, type = null) => {
    ...
    const getDetailData = () => {
      for (const property in dataGraphTwo[type ? type : typeFilter]
        .claims_received) {
        categories.push(property);
        newSeries.claims_received.push(
          dataGraphTwo[type ? type : typeFilter].claims_received[property]
        );
      }
      for (const property in dataGraphTwo[type ? type : typeFilter]
        .claims_resolved) {
        newSeries.claims_resolved.push(
          dataGraphTwo[type ? type : typeFilter].claims_resolved[property]
        );
      }
    };
    ...
    const formatCategoriesMouths = () => {
      for (var i = 0; i < categories.length; i++)
        categories[i] = `${month()[categories[i].split("-")[1]]} ${
          categories[i].split("-")[0][2]
        }${categories[i].split("-")[0][3]}`;
    };

    const formatCategoriesDays = () => {
      for (var i = 0; i < categories.length; i++) {
        categories[i] = `${categories[i].split("-")[2]} ${
          month()[categories[i].split("-")[1]]
        } ${categories[i].split("-")[0][2]}${categories[i].split("-")[0][3]}`;
      }
    };
    ...
    const formatCategoriesWeeks = () => {
      ...
      var end = "";
      for (var i = 0; i < categories.length; i++) {
        start = `${categories[i].replace(/\s/g, "").split("-")[2]} ${
          month()[categories[i].replace(/\s/g, "").split("-")[1]]
        } ${categories[i].replace(/\s/g, "").split("-")[0][2]}${
          categories[i].replace(/\s/g, "").split("-")[0][3]
        }`;
        end = `${categories[i].replace(/\s/g, "").split("-")[5]} ${
          month()[categories[i].replace(/\s/g, "").split("-")[4]]
        } ${categories[i].replace(/\s/g, "").split("-")[3][2]}${
          categories[i].replace(/\s/g, "").split("-")[3][3]
        }`;
    ...
    if (type === null) {
      if (typeFilter === "months") formatCategoriesMouths();
      else if (typeFilter === "days") formatCategoriesDays();
      else formatCategoriesWeeks();
    } else {
      if (type === "months") formatCategoriesMouths();
      else if (type === "days") formatCategoriesDays();
      else formatCategoriesWeeks();
    }
  ...
  const renderChart = (dataGraphOne, dataGraphTwo) => {
    const chartOne = new ApexCharts(
      ...
      getGraphOneOptions(dataGraphOne)
    );
    const chartTwo = new ApexCharts(
      document.getElementById("graphTwo"),
      getGraphTwoOptions(dataGraphTwo)
    );
    ...
    const fetchData = async () => {
      await axios
        ...
        .then((response) => {
          ...
          renderChart(
            response.data.statistiqueChannel,
            response.data.statistiqueGraphePeriod
          );
        })
  ...
  const handleTypeFilterChange = (e) => {
    ...
    const chartTwo = new ApexCharts(
      document.getElementById("graphTwo"),
      getGraphTwoOptions(fetchData.statistiqueGraphePeriod, e.target.value)
    );
    setStateChartTwo(chartTwo);
    chartTwo.render();
  };
    ...
    if (verifyTokenExpire()) {
      axios
        ...
        .catch((error) => {
          ...
          ToastBottomEnd.fire(
            toastErrorMessageWithParameterConfig(
              error.response.data.error.date_end
            )
          );
        });
    }
  ...
  const filterData = () => {
    if (institution && startDate && endDate) {
      ...
      ToastBottomEnd.fire(
        toastErrorMessageWithParameterConfig(
          t("Veillez renseigner les paramètres correctement")
        )
      );
    }
  };
  ...
  const formatPdfTitle = () => {
    ...
    else return `reporting_${new Date().getFullYear()}`;
  };
  ...
  const exportToPdf = async () => {
    ...
    if (fetchData) {
      ...
      const sendData = {
        ...
        statistiqueQualificationPeriod:
          fetchData.statistiqueQualificationPeriod,
      ...
      ToastBottomEnd.fire(
        toastErrorMessageWithParameterConfig(
          t("Donnée indisponible pour l'exportation")
        )
      );
    }
  };
      ...
      >
        ...
        <div className="kt-container  kt-container--fluid  kt-grid__item kt-grid__item--fluid">
            ...
            <div className="alert-icon">
              {startExportation ? (
                ...
                <button
                  ...
                  onClick={exportToPdf}
                >
                  <i className="fa fa-file-export" style={{ color: "white" }} />{" "}
                  {t("Exportez")}
                </button>
              )}
            </div>
                  ...
                  <div className="form-group col-3">
                    ...
                    <Select
                      ...
                      options={formatSelectOption(
                        fetchData ? fetchData.institutions : [],
                        "name",
                        false
                      )}
                    />
          ...
          <div className="kt-portlet">
            <HeaderTablePage title={"Reporting Reclamation"} />
                        ...
                        <thead style={{ backgroundColor: "#7F9CF5" }}>
                          <tr role="row">
                            <th
                              ...
                              rowSpan="1"
                              colSpan="1"
                              style={{ width: "70.25px", color: "white" }}
                              aria-label="Country: activate to sort column ascending"
                            >
                              {t("Catégorie de")} <strong>R</strong>
                            </th>
                            <th
                              tabIndex="0"
                              aria-controls="kt_table_1"
                            ...
                            <th
                              ...
                              aria-label="Country: activate to sort column ascending"
                            >
                              <strong>R</strong> {t("à assigner à une unité")}
                            </th>
                            <th
                              tabIndex="0"
                              aria-controls="kt_table_1"
                              ...
                              aria-label="Country: activate to sort column ascending"
                            >
                              <strong>R</strong> {t("à assigner à un agent")}
                            </th>
                            <th
                              tabIndex="0"
                              aria-controls="kt_table_1"
                              ...
                              indexEleRow === 0 ? (
                                <tr key={index}>
                                  {row.claim_objects.length === 0 ? (
                                    ...
                                    <td rowSpan={row.claim_objects.length}>
                                      {row.name["fr"]}
                                    </td>
                                  )}
                                  <td>{elRow.name["fr"]}</td>
                        ...
                        >
                          <tr role="row">
                            <th
                              ...
                              aria-label="Country: activate to sort column ascending"
                            >
                              {t("Délai de qualification")} ({t("en jours")})
                            </th>
                            {fetchData.statistiqueQualificationPeriod.map(
                              (el, index) => (
                                <th
                                  key={index}
                                  tabIndex="0"
                        ...
                        <thead style={{ backgroundColor: "#7F9CF5" }}>
                          <tr role="row">
                            <th
                              ...
                              aria-label="Country: activate to sort column ascending"
                            >
                              {t("Délai de traitement")} ({t("en jours")})
                            </th>
                            {fetchData.statistiqueTreatmentPeriod.map(
                              (el, index) => (
                                <th
                                  key={index}
                                  tabIndex="0"
          ...
          <div className="kt-portlet">
            <HeaderTablePage title={t("Utilisation de canaux")} />
                  ...
                  {fetchData ? (
                    <div className="col-sm-12">
                      ...
                      >
                        <thead style={{ backgroundColor: "#7F9CF5" }}>
                          <tr role="row">
                            {fetchData.statistiqueChannel.map(
                              (channel, index) => (
                                <th
                                  key={index}
                                  tabIndex="0"
          ...
          <div className="kt-portlet">
            <HeaderTablePage title={t("Évolution des réclamations")} />
                  ...
                  <div className="form-group col-6">
                    ...
                    >
                      <option value="" disabled={true}>
                        {t("Veillez sélectionner le filtre")}
                      </option>
