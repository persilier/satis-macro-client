...
const ClaimListDetail = (props) => {
  ...
  const formatStaffsOptions = (data) => {
    ...
    for (let i = 0; i < data.length; i++)
      newData.push({
        value: data[i].id,
        label: `${data[i].identite.lastname} ${data[i].identite.firstname}`,
      });
    return newData;
  };
  ...
  useEffect(() => {
    async function fetchData() {
      await axios
        .get(`${appConfig.apiDomaine}/claim-awaiting-treatment/${id}/edit`)
        .then((response) => {
  ...
  const selfAssign = () => {
    AssignClaimConfirmation.fire(confirmAssignConfig()).then((response) => {
      if (verifyTokenExpire()) {
        if (response.value) {
          axios
            .put(
              `${appConfig.apiDomaine}/claim-awaiting-treatment/${id}/self-assignment`,
              {}
            )
            .then((response) => {
    ...
    if (verifyTokenExpire()) {
      axios
        .put(
          ...
          { staff_id: staff ? staff.value : null }
        )
        .then((response) => {
          ToastBottomEnd.fire(toastAssignClaimSuccessMessageConfig());
