...
const ConfigRapportAuto = (props) => {
  ...
  document.title =
    "Satis rapport - " +
    (ready ? t("Paramètre configuration rapport automatique") : "");
  if (
  ...
  const filterShowListBySearchValue = (value) => {
    ...
    newRapportAuto = newRapportAuto.filter(
      (el) =>
        getLowerCaseString(
          el.institution_targeted ? el.institution_targeted.name : ""
        ).indexOf(value) >= 0 ||
        getLowerCaseString(el.period ? el.period_tag.label : "").indexOf(
          value
        ) >= 0 ||
  ...
  const deleteCategoryClient = (rapportAutoId, index) => {
    DeleteConfirmation.fire(confirmDeleteConfig()).then((result) => {
      if (verifyTokenExpire()) {
        if (result.value) {
          axios
            .delete(endPoint.list + `/${rapportAutoId}`)
            .then((response) => {
  ...
  const printBodyTable = (rapport, index) => {
    return (
      <tr key={index} role="row" className="odd">
        {props.plan === "MACRO" ? (
          <td>
            {rapport.institution_targeted
              ? rapport.institution_targeted.name
              : ""}
          </td>
        ) : null}
        {props.plan === "PRO" ? (
          <td>
            {rapport.staffs
              ? rapport.staffs.map(
                  (staff, index) =>
                    staff.identite.firstname +
                    " " +
                    staff.identite.lastname +
                    " ,"
                )
              : null}
          </td>
        ...
        <td className={"text-center"}>
          {(rapport?.period
            ? rapport.period
            : rapport?.period_tag && rapport.period_tag.label
            ? rapport.period_tag.label
            : "") === "biannual" ? (
        ...
        <td>
          {rapport.reporting_type === null ? "-" : rapport.reporting_type}
        </td>
        <td>
          {rapport.staffs
            ? rapport.staffs.map((staff, index) =>
                staff.identite.email
                  ? staff.identite.email.map((email, index) => email + " ")
                  : null
              )
            : null}
        </td>
  ...
  return ready ? (
    ...
    >
      ...
      <div className="kt-container  kt-container--fluid  kt-grid__item kt-grid__item--fluid">
        <InfirmationTable information={t("Liste des rapports automatiques")} />
