...
const UserAdd = (props) => {
  ...
  if (
    !(
      ...
      verifyPermission(props.userPermissions, "store-user-my-institution")
    )
  )
  ...
  const activePilot = institution
    ? institution.value === props.activeUserInstitution
    : "" === props.activeUserInstitution;
      ...
      await axios
        ...
        .then(({ data }) => {
          if (
            ...
            setIdentities(
              formatSelectOption(
                formatIdentities(data.identites),
                "fullName",
                false
              )
            );
  ...
  const formatIdentities = (identityList) => {
    for (let i = 0; i < identityList.length; i++)
      identityList[
        i
      ].fullName = `${identityList[i].lastname} ${identityList[i].firstname}`;
    return identityList;
  };
  ...
  const loadStaff = async (institutionId) => {
    if (institutionId) {
      if (verifyTokenExpire()) {
        await axios
          .get(`${appConfig.apiDomaine}/any/users/${institutionId}/create`)
          .then(({ data }) => {
            setRoles(formatSelectOption(data.roles, "name", false, "name"));
            setIdentities(
              formatStaff(formatIdentities(data.identites), "fullName")
            );
          })
  ...
  const onSubmit = async (e) => {
    ...
    if (verifyTokenExpire()) {
      if (
        (data.roles.includes("pilot-filial") ||
          data.roles.includes("pilot-holding")) &&
        activePilot
      ) {
        ...
        if (addUser) {
          if (data.activate === 1) {
            ...
            const active = await axios
              .put(
                `${appConfig.apiDomaine}/active-pilot/institutions/${data.institution_id}`,
            ...
            if (addUser && active) {
              ToastBottomEnd.fire(
                toastSuccessMessageWithParameterConfig(
                  t("Utilisateur enregistré et désigné comme pilote")
                )
              );
            }
            ...
            if (addUser && !active) {
              ToastBottomEnd.fire(
                toastSuccessMessageWithParameterConfig(
                  t("Utilisateur enregistré mais non défini comme pilote actif")
                )
              );
            }
          } else {
                    ...
                    <div className="kt-portlet__body">
                      {verifyPermission(
                        ...
                        >
                          ...
                          <div className="col-lg-9 col-xl-6 pl-2">
                            <Select
                              isClearable
                              value={institution}
                              placeholder={"SATIS"}
                            ...
                            <div className="kt-radio-inline">
                              <label className="kt-radio">
                                <input
                                  ...
                                  checked={optionOne === data.activate}
                                />{" "}
                                {t("OUI")}
                                <span />
                              </label>
                              <label className="kt-radio">
                                <input
                                  ...
                                  value={optionTwo}
                                  onChange={handleOptionChange}
                                  checked={optionTwo === data.activate}
                                />{" "}
                                {t("NON")}
                                <span />
                              </label>
                      ...
                      >
                        ...
                        <div className="col-lg-9 col-xl-6 kt-input-icon kt-input-icon--right pl-2">
                        ...
                        <div className="col-lg-9 col-xl-6 kt-input-icon kt-input-icon--right pl-2">
                          <span className="kt-input-icon__icon kt-input-icon__icon--right mt-3">
                            <i
                              ...
                              className="fa fa-eye-slash"
                              aria-hidden="true"
                              onClick={(e) => onViewPasswordConfirm(e)}
                            />
                          </span>
                          <input
