...
const RuleAddPage = (props) => {
        ...
        await axios
          ...
          .then((response) => {
            if (
              ...
              response.data.role.institution_types.map((el, index) =>
                newInstitutionTypes.push({ value: index, label: el })
              );
          ...
          .catch(({ response }) => {
            if (response.data && response.data.error)
              ToastBottomEnd.fire(
                toastErrorMessageWithParameterConfig(response.data.error)
              );
  ...
  const handleInstitutionType = (selected) => {
    const values = [];
    const newData = { ...data };
    if (selected) selected.map((el) => values.push(el.label));
  ...
  const handlePermissionChange = (e) => {
    ...
    else newPermission.push(e.target.name);
                    ...
                    <div className="kt-portlet__body">
                      ...
                      >
                        ...
                        <div className="col-lg-9 col-xl-6">
                          <input
                            ...
                            placeholder={t(
                              "Veuillez entrer la description du type d'unité"
                            )}
                      ...
                      {verifyPermission(
                        ...
                        <div
                          className={
                            error.permissions.length
                              ? "accordion  accordion-toggle-arrow validated"
  ...
  const onSubmit = (e) => {
    ...
    if (
      ...
      verifyPermission(props.userPermissions, "update-my-institution-type-role")
    )
