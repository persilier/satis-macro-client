...
const HistoricRevivals = (props) => {
  ...
  if (
    !verifyPermission(props.userPermissions, "list-unit-revivals") ||
    !verifyPermission(props.userPermissions, "list-staff-revivals")
  ) {
  ...
  const formatStaffsOption = function(options) {
    ...
    for (let i = 0; i < options.length; i++) {
      newOptions.push({
        value: options[i]["id"],
        label: `${options[i]["identite"]["firstname"]} ${options[i]["identite"]["lastname"]}`,
      });
    }
    return newOptions;
  };
  ...
  const printBodyTable = (revival, index) => {
    return (
      <tr key={index} role="row" className="odd">
        <td>{revival?.claim?.reference ? revival.claim.reference : ""}</td>
        <td>
          {`${
            revival?.created_by?.identite?.firstname
              ? revival.created_by.identite.firstname
              : ""
          } ${
            revival?.created_by?.identite?.lastname
              ? revival.created_by.identite.lastname
              : ""
          }`}
        </td>
        <td>{formatDateToTimeStampte(revival.created_at)}</td>
        <td>
        ...
        <td>
          {`${
            revival?.staff?.identite?.firstname
              ? revival.staff.identite.firstname
              : ""
          } ${
            revival?.staff?.identite?.lastname
              ? revival.staff.identite.lastname
              : ""
          }`}
        </td>
        ...
        <td style={{ textAlign: "center" }}>
          <HtmlDescription
            onClick={() => showModal(revival?.message ? revival.message : "-")}
          />
        </td>
        <td>
          {verifyPermission(props.userPermissions, "revive-staff") &&
          (props.userStaff.is_lead === true ||
            props.userStaff.is_active_pilot === true) ? (
            <>
              <a
                type="button"
                data-keyboard="false"
                ...
                className={`btn btn-sm btn-clean btn-icon btn-icon-md ${revival?.status ===
                  "considered" && "disabled"}`}
                title={t("Relancer")}
              >
              ...
              <RelaunchModal
                id={revival?.claim?.id ? revival.claim.id : ""}
                onClose={() => {}}
              />
            </>
          ) : null}
          {
            ...
            <a
              href={`/monitoring/claims/staff/${revival?.claim?.id}/detail`}
    ...
    >
      ...
      <div className="kt-container  kt-container--fluid  kt-grid__item kt-grid__item--fluid">
        <div className="kt-portlet">
          <HeaderTablePage title={t("Relances")} />
          ...
          <div className="kt-portlet__body">
            ...
            >
              {props.userStaff.is_lead === true ? (
                ...
                >
                  <label
                    className="col-xl-2 col-lg-2 text-right col-form-label"
                    htmlFor="host"
                  >
                    Agent
                  </label>
                  <div className="col-lg-7 col-xl-7">
                    ...
                    {error.staff.length
                      ? error.staff.map((error, index) => (
                          <div key={index} className="invalid-feedback mb-2">
                            {error}
                          </div>
                        ))
                      : null}
                  ...
                  <div className="col-xl-2 col-lg-2">
                    {load ? (
                      <button
                        className="btn btn-primary kt-spinner kt-spinner--left kt-spinner--md kt-spinner--dark ml-3"
                        type="button"
                        disabled
                      >
                        {"Chargement"}...
                  ...
                  <div className="row">
                    <div className="col-sm-12 col-md-5">
                      <div
                        ...
                        aria-live="polite"
                      >
                        {t("Affichage de")} 1{t("à")} {showList.length}{" "}
