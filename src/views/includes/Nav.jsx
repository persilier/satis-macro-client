...
const Nav = (props) => {
  ...
  const fetchData = async () => {
    if (verifyTokenExpire()) {
      await axios
        .get(`${appConfig.apiDomaine}/unread-notifications`)
        .then((response) => {
  ...
  useEffect(() => {
    if (props.user) {
      window.Echo.private(
        `Satis2020.ServicePackage.Models.Identite.${props?.user?.identite_id}`
      ).notification((notification) => {
        if (
          notification.type.substr(39, notification.type.length) ===
  ...
  const onClickLogoutLink = useCallback(
    (e) => {
      e.preventDefault();
      axios
        .get(`${appConfig.apiDomaine}/logout`)
        .then((response) => {
  ...
  const readAllNotification = async (readNotification, path) => {
    if (verifyTokenExpire()) {
      await axios
        .put(`${appConfig.apiDomaine}/unread-notifications`, readNotification)
        .then(({ data }) => {
          setStartRead(false);
          if (data.canReload) {
            ...
            setEventNotification(
              filterEventNotification(data.unreadNotifications)
            );
            setRelaunchNotification(
              filterRelaunchNotification(data.unreadNotifications)
            );
          }
        })
        .catch(({ response }) => {
  ...
  const notificationCount =
    eventNotification.length + relaunchNotification.length;
          ...
          <div className="kt-header__brand " id="kt_header_brand">
            <div className="kt-header__brand-logo">
              <a href="index.html">
                <img
                  alt="Logo"
                  src="/assets/images/satisLogo.png"
                  width={"100"}
                  height={"34"}
                />
                <span className="mx-2 text-white font-weight-bolder">
                  {props.plan}
                </span>
              </a>
            </div>
          ...
          <div className="kt-header__topbar">
                    ...
                    <div className="input-group">
                      ...
                      {loader ? (
                        <div className="mr-5 input-group-append">
                          <span className="input-group-text">
                            <i
                              className="kt-spinner kt-spinner--sm kt-spinner--brand"
                              style={{ display: "flex" }}
                            />
                          </span>
                        </div>
                              ...
                              <div className="d-flex align-items-center flex-grow-1 mb-2">
                                <div className="symbol symbol-30  flex-shrink-0">
                                  <div
                                    ...
                                    style={{
                                      backgroundImage:
                                        "url('https://preview.keenthemes.com/metronic/theme/html/demo1/dist/assets/media/users/300_20.jpg')",
                                    }}
                                  />
                                </div>
                                <div className="d-flex flex-column ml-3 mt-2 mb-2">
                                  <a
                                    href={`/process/claims/${searchData[0].reference}/detail`}
                                  ...
                                  <span className="font-size-sm font-weight-bold text-muted">
                                    {t("Reclamant") +
                                      ":" +
                                      searchData[0].claimer.lastname +
                                      " " +
                                      searchData[0].claimer.firstname}
                                  </span>
                                </div>
                              </div>
                            </div>
                  ...
                  <div className="kt-head kt-head--skin-light kt-head--fit-x kt-head--fit-b">
                    ...
                    >
                      ...
                      <button
                        ...
                        type="button"
                        className="btn btn-secondary btn-elevat  btn-sm bt-icon "
                          ...
                          {eventNotification.map((n, index) => (
                            <a
                              href={getNotificationLink(
                                n.type.substr(39, n.type.length),
                                n.data
                              )}
                              key={index}
                            ...
                            >
                              ...
                              <div className="kt-notification__item-details">
                                ...
                                >
                                  {n.data.text.length >= 87
                                    ? n.data.text.substring(0, 88) + "..."
                                    : n.data.text.substring(0, 86)}
                                </div>
                                <div className="kt-notification__item-time">
                                  {moment(new Date(n.created_at)).fromNow()}
                                </div>
                              </div>
                              {startRead ? <Loader /> : null}
                            </a>
                          ))}
                        </div>
                      ) : (
                        ...
                        >
                          {relaunchNotification.map((n, index) => (
                            <a
                              ...
                              onClick={(e) =>
                                showDetailNotification(e, ``, n.id, true)
                              }
                              className="kt-notification__item"
                            >
                              <div className="kt-notification__item-icon">
              ...
              {appConfig.useManyLanguage ? (
                <div className="dropdown-menu dropdown-menu-fit dropdown-menu-right dropdown-menu-anim">
                  <ul className="kt-nav kt-margin-t-10 kt-margin-b-10">
                    <li
                      className={`kt-nav__item ${i18n.language === "en" &&
                        "kt-nav__item--active"}`}
                    >
                      <a
                        href="#link"
                        onClick={(e) => onClickLanguage(e, "en")}
                        className="kt-nav__link"
                      >
                        <span className="kt-nav__link-icon">EN</span>
                        <span className="kt-nav__link-text">English</span>
                      </a>
                    </li>

                    <li
                      className={`kt-nav__item ${i18n.language === "fr" &&
                        "kt-nav__item--active"}`}
                    >
                      <a
                        href="#link"
                        onClick={(e) => onClickLanguage(e, "fr")}
                        className="kt-nav__link"
                      >
                        <span className="kt-nav__link-icon">FR</span>
            ...
            <div className="kt-header__topbar-item kt-header__topbar-item--user">
              ...
              >
                ...
                <span className="kt-header__topbar-icon kt-bg-brand kt-hidden">
                  <b>S</b>
                </span>
              </div>
              <div className="dropdown-menu dropdown-menu-fit dropdown-menu-right dropdown-menu-anim dropdown-menu-xl">
