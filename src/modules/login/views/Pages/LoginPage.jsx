...
const LoginPage = (props) => {
  ...
  const onClickConnectButton = async (e) => {
    ...
    await axios
      .post(appConfig.apiDomaine + `/login`, formData)
      .then((response) => {
      ...
      .catch((error) => {
        ...
        if (error.response.data.status === 403) {
          ...
          startDecounter(
            Math.trunc(error.response.data.expire_in / 60),
            error.response.data.expire_in % 60
          );
          ToastBottomEnd.fire(
            toastErrorMessageWithParameterConfig(error.response.data.message)
          );
          ...
          setExpireIn(null);
          let errorable = error?.response?.data?.message;

          if (
            errorable &&
            !errorable.includes("The provided authorization grant")
          ) {
            setError({
              username: ready ? t(errorable) : "",
              password: ready ? t(errorable) : "",
            });
            ToastBottomEnd.fire(
              toastErrorMessageWithParameterConfig(t(errorable))
            );
          } else {
            setError({
          ...
          setExpireIn(null);
          PasswordConfirmation.fire(
            passwordExpireConfig(error.response.data.message)
          ).then((response) => {
          ...
          ToastBottomEnd.fire(
            toastErrorMessageWithParameterConfig(
              t("Désolé, vous etes déjà connecté sur un autre appareil")
            )
          );
        } else {
          setExpireIn(null);
                ...
                <div className="kt-grid__item kt-grid__item--fluid kt-grid kt-grid--desktop kt-grid--ver-desktop kt-grid--hor-tablet-and-mobile">
                  <div
                    ...
                    style={{
                      backgroundImage: `url(${
                        componentData &&
                        componentData.params.fr.background.value
                          ? appConfig.apiDomaine +
                            componentData.params.fr.background.value.url
                          : " "
                      })`,
                    }}
                  >
                    <div className="kt-grid__item">
                      <span className="kt-login__logo">
                        <img
                          src={
                            componentData && componentData.params.fr.logo.value
                              ? appConfig.apiDomaine +
                                componentData.params.fr.logo.value.url
                        ...
                        >
                          {componentData
                            ? componentData.params.fr.version.value
                            : props.plan + appConfig.version}
                        </span>
                      </span>
                    </div>
                    <div className="kt-grid__item kt-grid__item--fluid kt-grid kt-grid--ver">
                      <div className="kt-grid__item kt-grid__item--middle">
                        <h3 className="kt-login__title">
                          {" "}
                          {componentData
                            ? componentData.params.fr.header.value +
                              componentData.params.fr.version.value
                            : null}
                        </h3>
                    ...
                    <div className="kt-login__body">
                      <Switch>
                        ...
                        <Route exact path={`/forgot-password/${tokenData}`}>
                          <ForgotPasswordForm token={tokenData} />
                        </Route>
                      </Switch>
                    </div>
                  </div>
