...
const endPointConfig = {
  ...
  MACRO: {
    holding: {
      ...
      storeAccount: (id) =>
        `${appConfig.apiDomaine}/any/accounts/${id}/clients`,
...
const HoldingClientForm = (props) => {
    ...
    if (verifyTokenExpire()) {
      axios.get(endPoint.create).then((response) => {
        ...
        if (
          ...
          const options = response.data.institutions.length
        ...
        if (
          ...
          const clientOptions = response.data.client_institutions?.data
            ? response.data.client_institutions.data.map((client) => ({
      ...
      if (id) {
        axios.get(endPoint.edit(id)).then((response) => {
          const newClient = {
            ...
            telephone:
              response.data.client_institution.client.identite.telephone,
            ...
            type_client:
              response.data.client_institution.client.identite.type_client,
            raison_sociale:
              response.data.client_institution.client.identite.raison_sociale,
          };
          setData(newClient);
          setType({
            value: response.data.account_type
              ? response.data.account_type.id
              : "",
            label: response.data.account_type
              ? response.data.account_type.name.fr
              : "",
          });
          setCategory({
            value: response.data.client_institution.category_client
              ? response.data.client_institution.category_client.id
              : "",
            label: response.data.client_institution.category_client
              ? response.data.client_institution.category_client.name.fr
              : "",
          });
        });
      }
  ...
  const onChangeClient = (selected) => {
    ...
    if (newData.client_id) {
      if (verifyTokenExpire()) {
        axios
          .post(endPoint.update(`${newData.client_id}`), newData)
          .then((response) => {
            const newIdentity = {
              ...
              ville:
                response.data.client.identite.ville === null
                  ? ""
                  : response.data.client.identite.ville,
            ...
            setData(newIdentity);
            setCategory({
              value: response.data.category_client
                ? response.data.category_client.id
                : "",
              label: response.data.category_client
                ? response.data.category_client.name.fr
                : "",
            });
          })
          .catch((error) => {
        ...
        axios
          ...
          .catch((errorRequest) => {
            ...
            if (errorRequest.response.data.error.message)
              ToastBottomEnd.fire(
                toastErrorMessageWithParameterConfig(
                  errorRequest.response.data.error.message
                )
              );
            else ToastBottomEnd.fire(toastEditErrorMessageConfig());
          });
      } else {
        if (data.client_id.length !== 0) {
          axios
            ...
            .catch((errorRequest) => {
              ...
              setError({
                ...defaultError,
                ...errorRequest.response.data.error,
              });
          ...
          axios
            ...
            .catch(async (errorRequest) => {
              if (errorRequest.response.data.error.identite) {
                ...
                ToastBottomEnd.fire(
                  toastErrorMessageWithParameterConfig(
                    errorRequest.response.data.client.identite.lastname +
                      " " +
                      errorRequest.response.data.client.identite.firstname +
                      ": " +
              ...
              <div className="kt-portlet__head">
                <div className="kt-portlet__head-label">
                  <h3 className="kt-portlet__head-title">
                    {id ? t("Modification de Clients") : t("Ajout de Clients")}
                  </h3>
                </div>
              </div>
                ...
                <div className="kt-portlet__body">
                  <div className="kt-section kt-section--first">
                    ...
                    <div className="form-group row">
                      ...
                      >
                        <label htmlFor="exampleSelect1">
                          {t("Catégorie Client")} <InputRequire />
                        </label>

                        {categoryClient ? (
                          <Select
                            placeholder={t(
                              "Veuillez sélectionner la catégorie client"
                            )}
                            value={category}
                      ...
                      <div className="form-group row">
                        ...
                        >
                          ...
                          <input
                            ...
                            disabled={
                              props.getDisable ? props.getDisable : false
                            }
                          />
                          {error.ville.length
                    ...
                    {data.type_client == "Physique" && (
                      <>
                        ...
                        <div className="form-group row">
                          <div
                            className={
                              error.sexe.length ? " col validated" : "col"
                            }
                          >
                            <label htmlFor="sexe">
                              {t("Sexe")}{" "}
                ...
                <div className="kt-portlet__body">
                  <div className="kt-section kt-section--first">
                    ...
                    <div className="form-group row">
                      <div
                        className={
                          error.account_type_id.length ? "col validated" : "col"
                        }
                      >
                        <label htmlFor="exampleSelect1">
                          {t("Type de compte")}
                        ...
                        {accountType ? (
                          <Select
                            placeholder={t(
                              "Veuillez selectionner le type de compte"
                            )}
                            value={type}
                      ...
                      <div
                        className={
                          error.number.length ? "col validated" : "col"
                        }
                      >
                        <label htmlFor="number">
                          {t("Numéro de Compte")} <InputRequire />
                        </label>
                        <input
                          id="number"
                          type="text"
